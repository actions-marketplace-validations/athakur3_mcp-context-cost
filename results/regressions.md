...
Every server with a measurement on record is in exactly one of the three sections below: **22 moved**, **64 held the same cost across every comparable measurement**, and **1 has no second comparable measurement yet** — 22 + 64 + 1 = 87.

**16 servers moved upward and 6 moved down**, a net +538 tokens across the measured set. 7 movements clear both thresholds for being called out (at least 5% *and* at least 25 tokens — relative alone would headline a fifth of a cheap server, absolute alone would headline drift on an expensive one). Everything comparable is listed either way.
...
The **release** column is what the two servers reported at `initialize`, on the two days either side of the movement. `—` means at least one of those rows does not record a version: either the server reports none, or the row was written before `history.csv` carried the column, and neither is something to fill in with a guess. 6 of 22 movements can name both sides today; the rest fill in as the rotation re-measures them.

**still `x`** means the cost moved while the version did not — 2 movements here. That is not a contradiction: an unpinned dependency can change a server's tool definitions without the server itself cutting a release, and the context window pays for it either way.
...
| [memory](../docs/servers/memory.md) | 2026-08-16 → 2026-09-21 | 2,378 | 9 | 12 |
